...
from backend.models import Film as FilmSchema
...
def like_film(film_id: int, request: Request):
    ...
    if cursor.fetchone():
        conn.close()
        return {"message": "Zaten beğendiniz."}  # 400 vermek yerine mesaj döndür
...
def rate_film(film_id: int, data: RatingInput, request: Request):
    username = get_username_from_token(request)
    user_id = get_user_id(username)
    score = data.score
...
def get_user_status(film_id: int, request: Request):
    ...
    return {"liked": liked, "user_score": score[0] if score else None}
...
def insert_sample_movies():
    ...
    for i, (title, year, rating) in enumerate(films, start=1):
        ...
        cursor.execute(
            "INSERT INTO films (id, title, year, rating, poster_url) VALUES (?, ?, ?, ?, ?)",
